...
func TestFileTransferEventStatusTimestampsTerminalEventsOnly(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{name: "started", event: session.Event{Timestamp: timestamp, Type: session.EventFileTransferStarted, Metadata: map[string]any{"source_path": "go.mod", "requested_path": "/tmp/go.mod"}}, want: "[16:08:12] [ChannelTerm] File transfer started: go.mod -> /tmp/go.mod\r\n"},
