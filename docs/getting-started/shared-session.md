...
An existing shared Session can also carry a CLI-only [file transfer](file-transfer.md). Use `Ctrl+] f` in an attachment for a guided local flow, or run `channelterm file send` / `channelterm file receive` from another terminal. During that transfer, the Host holds a `file-transfer` lease: attach input and ordinary `terminal_write` calls fail immediately rather than interleaving with the shell protocol.
