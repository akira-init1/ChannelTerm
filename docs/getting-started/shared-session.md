...
An existing shared Session can also carry a CLI-only [file transfer](file-transfer.md). Use `Ctrl+] f` in an attachment for a guided local flow, or run `channelterm file send` / `channelterm file receive` from another terminal. During a shortcut transfer, that attachment's input dispatcher locally discards ordinary keys, including Enter, before they can call `terminal_write`; it displays one input-ignored hint and reserves `Ctrl+C` for safe transfer cancellation. The Host still holds a `file-transfer` lease as the final protection: independent MCP `terminal_write` calls fail immediately rather than interleaving with the shell protocol, while the transfer's leased writes continue.
