...
If the command is interrupted while a send chunk is waiting for bytes, ChannelTerm pads that announced interval and waits for the shell to restore its saved TTY mode before releasing the lease. Directory-send cancellation uses deliberately invalid padding so a partial staging directory cannot be committed; the target pipeline drains the complete announced stream even after `tar` rejects that padding. If the Session or device disappears, reconnect locally and run `stty sane` on the board console if its terminal mode was not restored.
