...
- Pressing `Ctrl+]` enters local escape mode and displays the available escape commands locally; press `Esc` to cancel that local mode without sending a byte. `Ctrl+] t` toggles this CLI instance's prompt timestamps, which are off by default. The timestamp uses local `[HH:MM:SS]` format and appears only before conservatively recognized shell prompts, not ordinary output lines. This feedback is not Session output.
